import { c as createCommonjsModule, a as commonjsGlobal } from './admin.js';
...
codemirror.defineMode('markdown', (cmCfg, modeCfg) => {
  ...
  function blockNormal(stream, state) {
    ...
    state.indentedCode = false;
    let lineIndentation = state.indentation; // compute once per line (on first token)
    ...
    let allowsInlineContinuation = !prevLineLineIsEmpty && !prevLineIsHr && !state.prevLine.header && (!prevLineIsList || !prevLineIsIndentedCode) && !state.prevLine.fencedCodeEnd;
    let isHr = (state.list === false || prevLineIsHr || prevLineLineIsEmpty) && state.indentation <= maxNonCodeIndentation && stream.match(hrRE);
    let match = null;
  ...
  function inlineNormal(stream, state) {
    ...
    let ch = stream.next(); // Matches link titles present on next line
    ...
    if (modeCfg.xml && ch === '<' && stream.match(/^\/\w*?>/)) {
      ...
      let after = stream.peek() || ' '; // See http://spec.commonmark.org/0.27/#emphasis-and-strong-emphasis
